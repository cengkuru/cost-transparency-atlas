...
## Infrastructure portrait, 2026-09-06

The explorer now answers five publisher-specific questions: project population, published project budgets, infrastructure mix, reported project stage and procurement method. The prior field-completeness prongs and cross-publisher comparison are removed from this lens. The detailed record explorer remains a separate record layer. Connecting lines do not encode quantities; small category bars use their own displayed population, with no composite score or common radial scale.

Definition authority: OC4IDS v0.9.5 schema reference, https://standard.open-contracting.org/infrastructure/latest/en/reference/schema/, read 6 September 2026. Project budgets are distinct from contract values and final expenditure. Stage labels are publisher statements, not independent delivery verification. An open procurement method does not establish actual competition. Native currencies remain separate; USD appears only when explicitly published as USD. For mixed currencies, the headline shows the currency used by the most source records, with contributing population and other currencies indicated; exact separate totals are in the inspector. Counts of distinct published IDs do not establish unique physical assets.

Observed source checks: all ten licensed cached sources matched the immutable snapshot's SHA-256; Ecuador's streamed NDJSON matched its derived SHA-256. Record counts and contracting-process entry totals reconciled for every publisher. The reproducible generator is `node scripts/procurement-evidence.mjs`; it requires the cached sources and fails on mismatched hashes or populations. It regenerated data/procurement-evidence.json. Exact cross-level status logic is recorded there and tested independently: count each project once when project.status is exactly completion and any process summary.status is exactly active or pre-award. No excluded West Lombok project content is processed.

Ghana source population: 87 records with 87 distinct IDs; education appears in 33; all 87 project status fields report completion; no numeric project budget values with currencies; 89 open and five selective procedures across 94 entries. The exact cross-level rule identifies 45 project records, which is a reason to inspect their reporting, not proof of failed delivery. Independent Python Decimal calculation of Zambia raw project budgets confirms USD 20,710,741,193.5 across 71 records, ZMW 2,722,813,832.34 across seven, CNY 400,000,000 across one and EUR 114,500,000 across two. These are partial published budget totals, not all-project investment values.

New pure-module tests were observed red before implementation and repair, then green. The complete suite passes 25 tests. The data-analysis skill named by the older router was unavailable; the installed data-quality workflow and deterministic raw-source checks supplied the validity checks. No synthetic data, exchange-rate service, estimation or cross-country normalization is introduced.
...
## Public road-project story, 6 September 2026

The landing page now follows the routine-maintenance publication for road R762 between Mocímboa da Praia and Palma, Mozambique. The global Atlas opening remains separate. The earlier Zambia field-completeness chapters are removed from the landing story; their saved evidence files remain available for existing links. The page is an explanatory decision layer; its source extract is the record layer.

Primary verification against the original cached publisher package confirmed SHA256 `195b5a34214d8358b2dd44e90bcf611b10b06321c9359b67f2ead790999da194`, matching the immutable 5 September snapshot. All 16 quoted source paths in `data/story/road-project.json` were independently resolved and compared to the original JSON. The published contracting-process title specifies 81 km. Project budget and contract value each state MZN 13,399,664.88 and are not added. Four reported transactions sum to MZN 1,989,847.20 using decimal arithmetic; the latest transaction date is 7 July 2026. These are publisher statements, without independent inspection or payment-receipt verification. Project `identification` and process `active` are labels at different levels and are not treated as a contradiction.

The source-contract regression test was observed failing before the evidence file existed, then passing. CI checks the committed sanitized source extract, its linkage to the immutable snapshot hash, separate amount fields, dates, roles and payment calculation. The ignored raw cache is deliberately not required in CI or copied into the public build. This avoids claiming that CI authenticates the entire original package; that check was performed locally against the recorded hash.

Final verification: 26 tests passed on Node 22; build, script syntax and whitespace checks passed. Independent review passed after the schedule sentence was narrowed to the project period quoted in the saved extract. Native Chrome inspection confirmed all five changing visual states, legacy identifiers/record/action links resolving to cost/schedule/questions, and the project evidence drawer with correct amounts and source details. Screenshots at desktop, 320 by 844 and 390 by 844 showed the inspected chapters fitting without visible horizontal overflow; the schematic disclaimer remains visible on phones. No claim of exhaustive accessibility or all-device coverage is made.
